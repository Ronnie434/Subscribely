...
export default function OneTimeChargeModal({
  visible,
  item,
  onPaid,
  onSkipped,
  ...
  const [processingType, setProcessingType] = useState<'paid' | 'skipped' | 'dismissed' | null>(null);
  ...
  const handleDismiss = async () => {
    ...
    Alert.alert(
      'Dismiss Forever?',
      `Are you sure you want to permanently dismiss "${item.name}"? This charge will no longer appear in your past due list.`,
      [
        {
          text: 'Cancel',
          style: 'cancel',
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.error('Error dismissing charge:', error);
              if (Platform.OS === 'ios') {
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
              }
              Alert.alert(
                'Error',
                'Failed to dismiss charge. Please try again.',
                [{ text: 'OK' }]
              );
            } finally {
              setLoading(false);
    ...
    <Modal
      ...
      <View style={styles.overlay}>
        <View style={styles.modal}>
          ...
          {/* Title */}
          <Text style={styles.title}>One-Time Charge Due</Text>
          <Text style={styles.subtitle}>
            Did you pay for this charge?
          </Text>

          {/* Item Details */}
          <View style={styles.detailsCard}>
            <View style={styles.oneTimeBadge}>
              <Text style={styles.oneTimeBadgeText}>One-Time Only</Text>
            </View>
          ...
          <View style={styles.buttonContainer}>
            ...
            <AnimatedPressable
              ...
              style={[
                ...
                loading && processingType !== 'dismissed' && styles.loadingContainer,
              ]}
              disabled={loading}
              scaleOnPress={0.98}>
              {loading && processingType === 'dismissed' ? (
                <ActivityIndicator color={theme.colors.error} />
              ) : (
                <>
                  <Ionicons name="trash-outline" size={18} color={theme.colors.error} />
                  <Text style={styles.dismissButtonText}>Dismiss Forever</Text>
                </>
              )}
            </AnimatedPressable>
          </View>
